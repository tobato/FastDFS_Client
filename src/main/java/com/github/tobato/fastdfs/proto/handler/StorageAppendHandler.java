...
public class StorageAppendHandler extends AbstractHandler<Void> {

    private static final byte cmd = CmdConstants.STORAGE_PROTO_CMD_APPEND_FILE;
     ...
     * @param ext
     */
    public StorageAppendHandler(Socket socket, InputStream ins, long size, String path, Charset charset) {
        super(socket);
        this.ins = ins;
        this.size = size;
    ...
    protected void send(OutputStream ous) throws IOException {
        ...
        body_len = 2 * OtherConstants.FDFS_PROTO_PKG_LEN_SIZE + pathBytes.length + size;
